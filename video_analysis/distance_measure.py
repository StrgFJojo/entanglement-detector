...
def get_distance(personwiseKeypoints, indices_of_two_highest_scores, keypoints_list, synch_style):
    ...
    for j in range(len(indices_of_two_highest_scores)):
        ...
        keypoint_id_rhip = personwiseKeypoints[indices_of_two_highest_scores[j]][8]  # id is also index
        ...
        keypoint_id_lhip = personwiseKeypoints[indices_of_two_highest_scores[j]][11]  # id is also index
...
def get_distance_multipax(personwiseKeypoints, keypoints_list, person_indices):
    ...
    for j in range(len(person_indices)):
        ...
        # get coordinates of right hip
        keypoint_id_rhip = personwiseKeypoints[person_indices[j]][8]  # id is also index
        if keypoint_id_rhip == -1:
            x_rhip = -1
            y_rhip = -1
        else:
        ...
        # get coordinates of left hip
        keypoint_id_lhip = personwiseKeypoints[person_indices[j]][11]  # id is also index
        if keypoint_id_lhip == -1:
            x_lhip = -1
            y_lhip = -1
        else:
    ...
    if len(center_points) > 1:
        center_points = sorted(center_points, key=lambda x: x[0]) # sorted, ascending x coordinates
        distance_midhip = 0
        for k in range(len(center_points)-1):
            distance_midhip += abs(dist.euclidean(center_points[k], center_points[k+1]))
    else:
        distance_midhip = -1
...
def new_get_distance(pose_entries_selected, all_keypoints, synch_style):
    #print("persons", len(indices_of_two_highest_scores))
    # center points of their bodies
    center_points = np.zeros((len(pose_entries_selected), 2))
    distance_midhip = -1
    rhip_coordinates = [all_keypoints[x[8]][:1] for x in pose_entries_selected]
    lhip_coordinates = [all_keypoints[x[11]][:1] for x in pose_entries_selected]
    # for both skaters
    for j in range(len(pose_entries_selected)):
