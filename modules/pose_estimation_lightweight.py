        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            if (
                (
                    ...
                    and synch_degree[part_id] == -1
                )
                or (
                    idx not in person_indices
                    and synch_style
                    in [
                        "2pax_90",
                        "2pax_180",
                ...
                or (synch_style == "allpax" and synch_degree[part_id] == -1)
            ):
                color = [0, 0, 0]
            elif (
                synch_style in ["2pax_90_mirrored", "2pax_180_mirrored"]
                and idx == person_indices[1]
            ):
                color = [
                    0,
                    min(
                        ...
                        2
                        * 255
                        * (synch_degree[self.idx_mirror_pose_pair[part_id]]),
                    ),
                    min(
                        255,
                        2
                        * 255
                        * (
                            1 - synch_degree[self.idx_mirror_pose_pair[part_id]]
                        ),
                    ),
                ]
            else:
                color = [
                    0,
                    min(255, 2 * 255 * (synch_degree[part_id])),
