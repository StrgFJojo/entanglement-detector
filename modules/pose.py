...
class Pose:
    ...
    def draw(self, img, synch_degree, person_indices, synch_style, idx):
        assert self.keypoints.shape == (Pose.num_kpts, 2)

        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            if (idx in person_indices and synch_style in ['2pax_90', '2pax_180', '2pax_90_mirrored', '2pax_180_mirrored'] and synch_degree[part_id] == -1) or (idx not in person_indices and synch_style in ['2pax_90', '2pax_180', '2pax_90_mirrored', '2pax_180_mirrored']) or (synch_style == 'allpax' and synch_degree[part_id] == -1):
                color = [0, 0, 0]
            elif synch_style in ['2pax_90_mirrored', '2pax_180_mirrored'] and idx == person_indices[1]:
                color = [0, min(255, 2 * 255 * (synch_degree[self.idx_mirror_pose_pair[part_id]])),
                         min(255, 2 * 255 * (1 - synch_degree[self.idx_mirror_pose_pair[part_id]]))]
            else:
                color = [0, min(255, 2 * 255 * (synch_degree[part_id])),
                         min(255, 2 * 255 * (1 - synch_degree[part_id]))]
