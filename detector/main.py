...
def run(
    ...
    if save_input_video:
        ...
        output_video_path2 = os.path.join(
            ROOT_DIR, "experiments/raw_videos", "output%d.avi" % exp_num
        )
        frame_width2 = int(iterable.cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        frame_height2 = int(iterable.cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
        fps2 = int(iterable.cap.get(cv2.CAP_PROP_FPS))
        fourcc2 = cv2.VideoWriter_fourcc(*"MJPG")
        writer2 = cv2.VideoWriter(
            output_video_path2, fourcc2, fps2, (frame_width2, frame_height2)
        )
    ...
    for img in image_provider:
        ...
        if len(pose_entries) > 1:
            ...
            if synch_style in [
                ...
                try:
                    ...
                    print(
                        "%s is not a valid input for argument synch_style"
                        % synch_style
                    )
                    exit(1)
            person_indices = (-scores).argsort()[:pax]

            synch_degree = synchrony_measure.get_synchrony_flexpax(
                pose_entries,
                person_indices,
            ...
            distance_midhip = distance_measure.get_distance_multipax(
                pose_entries, all_keypoints, person_indices
            )
            heights = height_measure.get_height_multipax(
                pose_entries, all_keypoints, person_indices
            )

            if -1 in heights or sum(heights) <= 0:
                normalized_distance = -1
            else:
                normalized_distance = distance_midhip / (sum(heights))

            if save_outputs:
                synchrony_totalvideo = np.vstack(
                    (synchrony_totalvideo, synch_degree)
                )
                normalized_distances_totalvideo = np.append(
                    normalized_distances_totalvideo, normalized_distance
                )
        else:
            if save_outputs:  # -1
                synchrony_totalvideo = np.vstack(
                ...
                normalized_distances_totalvideo = np.append(
                    normalized_distances_totalvideo, -1
                )
        if track:
        ...
        if show_livestream:
            ...
            img = visualization.partly_overlay_final(
                pose_entries,
                person_indices,
                pose_estimation_openpose.POSE_PAIRS,
                all_keypoints,
    ...
    if save_outputs:
        ...
        col_names = list(
            (
                "synchrony_"
                + pose_estimation_openpose.keypointsMapping[
                    pose_estimation_openpose.POSE_PAIRS[t][0]
                ]
                + "_to_"
                + pose_estimation_openpose.keypointsMapping[
                    pose_estimation_openpose.POSE_PAIRS[t][1]
                ]
            )
            for t in range(len(pose_estimation_openpose.POSE_PAIRS))
        )
...
if __name__ == "__main__":
    ...
    checkpoint_path = os.path.join(
        ROOT_DIR, "models", "checkpoint_iter_370000.pth"
    )
    ...
    parser.add_argument(
        "--smooth", type=int, default=1, help="smooth pose keypoints"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="2pax_90",
        help="type of synchrony metric",
    )
    parser.add_argument(
        "--full-stats-mode", type=int, default=0, help="get all synch styles"
    )
    ...
    if args.save_outputs:
        df, synchrony_totalvideo, normalized_distances_totalvideo = run(
            net,
            frame_provider,
            args.height_size,
            args.cpu,
