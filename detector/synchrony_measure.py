    ...
    if synch_style in [
        ...
        for n in range(17):
            ...
            for i in range(
                ...
                # keypoints for (neck,r-shoulder)
                # of 1st person have ids 2 and 4
                if (
                    synch_style in ["2pax_90_mirrored", "2pax_180_mirrored"]
                    and i == 1
                ):
                    index = personwiseKeypoints[
                        indices_of_two_highest_scores[i]
                    ][np.array(POSE_PAIRS[idx_mirror_pose_pair[n]])]
                else:
                    index = personwiseKeypoints[
                        indices_of_two_highest_scores[i]
                    ][np.array(POSE_PAIRS[n])]
                ...
                x_vals = np.int32(
                    keypoints_list[index.astype(int), 0]
                )  # for both key points of pose pair
                y_vals = np.int32(
                    keypoints_list[index.astype(int), 1]
                )  # for both key points of pose pair
        ...
        for n in range(17):
            ...
            if len(unit_vectors) > 0:
                ...
                synch_degree[n] = -1
        synch_degree = [
            (float(x) / len(synch_degree)) ** 0.5 if x >= 0 else -1
            for x in synch_degree
        ]
        synch_degree = [
            (float(x) - min(synch_degree))
            / (max(synch_degree) - min(synch_degree))
            if x >= 0
            else -1
            for x in synch_degree
        ]
...
def get_synchrony_flexpax(
    pose_entries, person_indices, POSE_PAIRS, all_keypoints, synch_style
):
    synch_degree = -1 * np.ones(17)
    for n in range(17):
        ...
        for i in range(len(person_indices)):  # for each of the persons selected
            ...
            if (
                ...
                index = pose_entries[person_indices[i]][
                    np.array(POSE_PAIRS[idx_mirror_pose_pair[n]])
                ]
            else:
            ...
            # look up coordinates for keypoints
            x_vals = np.int32(
                all_keypoints[index.astype(int), 0]
            )  # for both key points of pose pair
            y_vals = np.int32(
                all_keypoints[index.astype(int), 1]
            )  # for both key points of pose pair
    ...
    if synch_style not in [
        ...
        synch_degree = [
            (float(x) / len(synch_degree)) ** 0.1 if x >= 0 else -1
            for x in synch_degree
        ]
        synch_degree = [
            (float(x) - min(synch_degree))
